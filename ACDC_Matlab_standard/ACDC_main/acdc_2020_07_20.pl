...
##         9.13 Coagulation sink for specific clusters from the command line
##         9.14 Printing out size-dependent sink in dry case
##         9.15 Hydrate average for size-dependent sink
##      9.2 Wall losses
...
if($help){
		...
		\t--cs_only clust,value
		\t\tuse for cluster 'clust' the coagulation loss 'value' instead of the parameterization or value used for other clusters\n\n
		MCMC options:\n
		\t--evap_coef_mcmc
...
if($lfortran){
	...
	if($lloop){
		...
		print OUT "\tinteger, save :: ind_quad_loss($max_n_flux,0:$ind_quad_loss[0])=0,ind_quad_form($max_n_flux,0:$ind_quad_form[0])=0\n";
		print OUT "\tinteger, save :: ind_quad_loss_extra($max_n_flux,0:$ind_quad_loss_extra[0])=0,ind_quad_form_extra($max_n_flux,0:$ind_quad_form_extra[0])=0\n";
		print OUT "\tinteger, save :: ind_lin_loss($max_n_flux,0:$ind_lin_loss[0])=0,ind_lin_form($max_n_flux,0:$ind_lin_form[0])=0,fitted($size_C_out,0:$size_C_out)=0\n";
		if($l_reac_in_clus){
			print OUT "\tinteger, save :: ind_reac_loss($max_n_flux,0:$ind_reac_loss[0])=0,ind_reac_form($max_n_flux,0:$ind_reac_form[0])=0\n";
		}
		print OUT "\treal(kind(1.d0)), save :: n_quad_form_extra($max_n_flux,0:" . ($ind_quad_form_extra[0])/3 . ")=0.d0\n";
	}
...
if($lfortran){
	...
	if($lloop){
		...
		print OUT "\treal(kind(1.d0)), save :: coef_quad(nclust,nclust,$max_n_flux)=0.d0,coef_lin($max_n_flux,$max_n_flux,nclust)=0.d0,source($size_C_out)=0.d0\n";
		print OUT "\tinteger, save :: charge(nclust)=0,ind_quad_loss($max_n_flux,0:$ind_quad_loss[0])=0,ind_quad_form($max_n_flux,0:$ind_quad_form[0])=0\n";
		print OUT "\tinteger, save :: ind_quad_loss_extra($max_n_flux,0:$ind_quad_loss_extra[0])=0,ind_quad_form_extra($max_n_flux,0:$ind_quad_form_extra[0])=0\n";
		print OUT "\tinteger, save :: ind_lin_loss($max_n_flux,0:$ind_lin_loss[0])=0,ind_lin_form($max_n_flux,0:$ind_lin_form[0])=0,fitted($size_C_out,0:$size_C_out)=0\n";
		print OUT "\tlogical, save :: isconst($size_C_out)=.false.\n";
	}
...
if($lfortran && !$lloop){
	...
	print OUT "\tinteger :: ind_quad_loss($max_n_flux,0:$ind_quad_loss[0]),ind_quad_form($max_n_flux,0:$ind_quad_form[0]),ind_quad_loss_extra($max_n_flux,0:$ind_quad_loss_extra[0]),ind_quad_form_extra($max_n_flux,0:$ind_quad_form_extra[0])\n";
	print OUT "\tinteger :: ind_lin_loss($max_n_flux,0:$ind_lin_loss[0]),ind_lin_form($max_n_flux,0:$ind_lin_form[0])\n";
