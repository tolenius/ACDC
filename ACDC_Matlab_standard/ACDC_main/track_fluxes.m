function [fluxes,clust_bound] = track_fluxes(clust_flux,clust,ch_info,flux,outflux,varargin)
% Finds the main flux chains through the system for a 2- or 3-component case
% Input:
...
% Max. numbers of molecules
nmols_max_def=[4,4,4];
nmols_max=nmols_max_def(1:length(name_mon));
if length(name_mon)==2
    if ismember('N',name_mon)
        nmols_max=[5,5];
    elseif ismember('M',name_mon)
        nmols_max=[3,3];
    elseif ismember('D',name_mon)
        nmols_max=[4,4];
    elseif ismember('T',name_mon)
        nmols_max=[2,2];
    end
end
...
if ~exist('fn','var')
    [~,~,roundoutname,roundout] = plotflux_out(clust,ch_info,outflux,'ch',ch,'out_channel',out_channel,'crit',crit_out);
else
    [~,~,roundoutname,roundout] = plotflux_out(clust,ch_info,outflux,'ch',ch,'out_channel',out_channel,'crit',crit_out,'fn',fn);
end
        ...
        if start_ch==ch
            if ~exist('fn','var')
                [~,~,mainfluxnames,mainfluxes]=plotflux(fluxes(i1).start,'to',clust_flux,clust,ch_info,flux,...
                    'crit',crit_clust,'lsame_ch');
            else
                [~,~,mainfluxnames,mainfluxes]=plotflux(fluxes(i1).start,'to',clust_flux,clust,ch_info,flux,...
                    'crit',crit_clust,'lsame_ch','fn',fn);
            end
...
%     line_coord=[nmols_max+1; nmols_max+1];
%     line_coord(1,nmon)=0;
%     if length(name_mon)==2
%         line(line_coord(:,1),line_coord(:,2),'Color','k','LineWidth',2.5,'LineStyle','--');
%     else
...
for i=1:size(fluxes,2)
    % Coordinates of the tracked cluster for possible text boxes
