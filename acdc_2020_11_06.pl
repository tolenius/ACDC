...
my(@cs_only,@cs_only_hydr,$one_cs_for_all,$one_cs_for_almost_all,@wl_only,@wl_only_hydr,$one_wl_for_all,$one_wl_for_almost_all);
...
my($variable_temp,$variable_cs,$variable_ion_source,$l_variable_parameters,$n_variable_parameters,$cs_value,$cs_value_in);
...
'save_outgoing'=>\$l_save_outgoing, 'save_coag_per_clust'=>\$l_save_coag_per_clust, 
...
'cs_only=s' => \@cs_only, 'wl_only=s' => \@wl_only, 
...
if($help){
		...
		print "\nSee ACDC manual at www.aces.su.se/staff/tinja-olenius/ for help.\n\n";
}
...
if(($use_coag_sinks+$use_wall_terms+$use_dilution+$variable_cs+$l_wl_mcmc)>0 || $coag_terms_in ne '' || $wall_terms_in ne '' || $#cs_only>=0 || $#wl_only>=0 || $#wl_mcmc_only>=0){
...
if($l_save_coag_per_clust && !$lfortran){
...
################################################################################
	...
	for($imol=0;$imol<$nmol_types;$imol++){
		print "molecule $imol:\t";
		print "name $molecule_name[$imol], charge $charge[$imol]\n";
	}
	print "\n";
}
...
if($l_save_outgoing){
    ...
    if($l_save_coag_per_clust){
        ...
        $size_ind_coef += $max_cluster_number;
    }
}else{
	$size_C_out = $max_cluster_number;
}
...
if(!$lloop){
	...
	for($iclus=$max_cluster_number+1;$iclus<=$size_C_out;$iclus++){
        if($l_save_coag_per_clust && $iclus > $max_n_flux){
            last;
        }
		$lines{'cl'} .= "'" . $label[$iclus] . "' ";
	}
	...
	if($lfortran){
		...
		print SYST "subroutine get_charging_state$append_to_subroutine_names(charging_state)\n\timplicit none\n";
		print SYST "\tinteger :: charging_state($max_cluster_number)\n\n\tcharging_state = (/$lines{'cst'}/)\n";
		print SYST "\nend subroutine get_charging_state$append_to_subroutine_names\n\n";
        ...
        if($l_save_coag_per_clust){
            print SYST "\n\t! Elements from ".($max_n_flux+1)." to $size_C_out: coagulation loss of each cluster\n";
            print SYST "\tj = 0\n";
            print SYST "\tdo i = ".($max_n_flux+1).", $size_C_out\n";
            print SYST "\t\tj = j+1\n";
            print SYST "\t\tclust(i)(:) = 'coag of '//trim(adjustl(clust(j)(:)))\n";
            print SYST "\tend do\n";
        }
...
if($lfortran){
	print SYST "subroutine monomer_names$append_to_subroutine_names(clust_mon)\n\timplicit none\n";
	...
	print SYST "\nend subroutine monomer_names$append_to_subroutine_names\n\n";
	...
	if($lloop && $nmol_types_used>1){
		...
		print SYST "\tcall get_cluster_numbers$append_to_subroutine_names(clust_from_indices)\n";
    ...
    if(!$lloop){
        ...
        print SYST "subroutine get_nmols_nocharge$append_to_subroutine_names(labels_nocharge,nmols_nocharge_clust)\n\timplicit none\n";
        print SYST "\tcharacter(len=11), dimension($jm) :: labels_nocharge\n\tinteger :: nmols_nocharge_clust($max_cluster_number,$jm)\n\n";
        $i = 0;
        for($imol=1;$imol<=$nmol_types_used;$imol++){
            if($charge[$mol_types_used[$imol]] == 0){
                $i += 1;
        ...
        for($iclus=1;$iclus <= $max_cluster;$iclus++){
            ...
            for($imol=1;$imol<=$nmol_types_used;$imol++){
                if($charge[$mol_types_used[$imol]] == 0){
                    $i += 1;
                    $temp_array[$i] += $clust_comp[$iclus][$mol_types_used[$imol]];
                    if($n_corr_negative_mol[$mol_types_used[$imol]] >= 0){
                        $temp_array[$i] += $clust_comp[$iclus][$n_corr_negative_mol[$mol_types_used[$imol]]];
                    }
                    if($n_corr_positive_mol[$mol_types_used[$imol]] >= 0){
                        $temp_array[$i] += $clust_comp[$iclus][$n_corr_positive_mol[$mol_types_used[$imol]]];
                    }
                }
            }
            print SYST "\tnmols_nocharge_clust($iclus,:) = (/" . join(', ',@temp_array) . "/)\n";
		}
        print SYST "\n";
        print SYST "end subroutine get_nmols_nocharge$append_to_subroutine_names\n\n";
        
        # Compositions and indices of clusters at the system boundary
        print SYST "subroutine get_bound$append_to_subroutine_names(bound_clusters,nmols_bound)\n\timplicit none\n";
        ...
        $str_temp =~ s/, $//;
        print SYST "\n" . $str_temp . "/)\n\n";
        print SYST "end subroutine get_bound$append_to_subroutine_names\n\n";
    }
...
if($lfortran){
	...
	if($lloop){
        ...
        if($l_save_coag_per_clust){
            print OUT "! ".($max_n_flux+1)."-$size_C_out are for coagulation losses of each cluster\n";
        }
		print OUT "!!!!!!!!!!!!!!!!!!!!!! END KEY !!!!!!!!!!!!!!!!!!!!!!\n\n";
	...
	if($lloop){
		if($nmol_types_used>1){
			print OUT "use acdc_system, only : monomer_indices\n";
		}
		print OUT "use acdc_simulation_setup, only : sources_and_constants\n";
		if($l_j_in){
	...
	if($lloop){
		...
		print OUT "\treal(kind(1.d0)), save :: coef_quad(nclust,nclust,$size_ind_coef)=0.d0,coef_lin($size_ind_coef,$size_ind_coef,nclust)=0.d0,source($size_C_out)=0.d0\n";
		...
		print OUT "\tinteger, save :: ind_lin_loss($size_ind_coef,0:$ind_lin_loss[0])=0,ind_lin_form($size_ind_coef,0:$ind_lin_form[0])=0,fitted($size_C_out,0:$size_C_out)=0\n";
		if($l_reac_in_clus){
	...
	if($lloop && ($nmol_types_used>1 || $l_j_in)){
		...
		if($nmol_types_used>1){
			...
			print OUT "\t\tcall get_molecule_numbers$append_to_subroutine_names(nclust_inc,indices)\n";
			print OUT "\t\tcall get_cluster_numbers$append_to_subroutine_names(clust_from_indices)\n";
			print OUT "\t\tcall monomer_indices$append_to_subroutine_names(n_monomers)\n";
			if($lloop_restrict){
	...
	if($lloop){
		print OUT "\t\tcall sources_and_constants($nclust_included,source,isconst,fitted)\n";
		if($l_mcmc_any){
			print OUT "\t\tcall get_coll$append_to_subroutine_names($nclust_included,K,coef)\n";
		}elsif( $variable_temp){
			print OUT "\t\tcall get_coll$append_to_subroutine_names($nclust_included,K,coef(1))\n";
		}else{
			print OUT "\t\tcall get_coll$append_to_subroutine_names($nclust_included,K)\n";
		}
		if(!$disable_evap){
			if($l_mcmc_any){
				print OUT "\t\tcall get_evap$append_to_subroutine_names($nclust_included,E,coef)\n";
			}elsif($variable_temp){
				print OUT "\t\tcall get_evap$append_to_subroutine_names($nclust_included,E,K,coef(1))\n";
			}else{
				print OUT "\t\tcall get_evap$append_to_subroutine_names($nclust_included,E)\n";
			}
		}
		...
		if($l_use_get_losses){
			print OUT "\t\tcall get_losses$append_to_subroutine_names($nclust_included,loss";
			if($variable_cs){
				print OUT ",coef";
			}
			print OUT ")\n";
...
if($lfortran && !$lloop){
	...
	print OUT "\tinteger :: fitted($size_C_out,0:$size_C_out)\n";	
	if($l_mcmc_any){
	...
	print OUT "\n\tcall sources_and_constants($size_C_out,source,isconst,fitted)\n";
	if($include_generic_neg && $include_generic_pos){
		if($generic_positive_ions_corr ne ''){
			print OUT "\n\t! setting positive charger ion concentration to give charge balance\n";
    ...
    if($l_save_coag_per_clust){
        print OUT "\n\t! ".($max_n_flux+1)."-$size_C_out: coagulation loss of each cluster\n";
        print OUT "\tj = 0\n";
        print OUT "\tdo i = ".($max_n_flux+1).", $size_C_out\n";
        print OUT "\t\tj = j+1\n";
        print OUT "\t\tind_lin_form(i,0:2) = (/ 1, i,j /)\n";
        print OUT "\tend do\n";
    }
    ...
    if($l_save_coag_per_clust){
        ...
        print OUT "\tdo i = ".($max_n_flux+1).", $size_C_out\n";
        print OUT "\t\tj = j+1\n";
        print OUT "\t\tcoef_lin(i,i,j) = cs(j)\t! cluster j -> coag of cluster j\n";
        print OUT "\tend do\n";
    }
...
if($lloop){
	...
	if($nmol_types_used>1){
		print OUT "subroutine get_molecule_numbers$append_to_subroutine_names(nclust_inc,indices)\n";
		print OUT "\timplicit none\n";
		print OUT "\tinteger, parameter :: nclust = $max_cluster_number\n";
		...
		print OUT "subroutine get_cluster_numbers$append_to_subroutine_names(clust_from_indices)\n";
		if($lloop_restrict){
			print OUT "use restriction_criteria\n";
			print "\n---------------------------------------------------------\n";
			print "\tYou will need the module restriction_criteria\n\tin order to compile and run the code.\n";
			print "---------------------------------------------------------\n\n";
		}
		print OUT "\timplicit none\n";
		...
		$string = "";
		if($lloop_restrict){
			print OUT $str_temp."call check_validity((/$nmol_ranges_ind/),valid)\n";
		...
		print OUT "\nend subroutine get_cluster_numbers$append_to_subroutine_names\n\n!-----------------------------------------------------------\n\n";
...
if($lfortran){
	if($l_use_get_losses){
		...
		if(!$lloop){
			...
			print OUT "subroutine get_losses$append_to_subroutine_names(nrates,loss";
			if($variable_cs){
				print OUT ",coef";
			}
			print OUT ")\n";
			...
			if($nmol_types_used>1){
				...
				print OUT "\t\tcall get_molecule_numbers$append_to_subroutine_names(nclust_inc,indices)\n";
				if($#cs_only>=0){
					print OUT "\t\tcall get_cluster_numbers(clust_from_indices)\n";
				}
				print OUT "\tend if\n";
			}
			print OUT "\n";
...
################################################################################
        ...
        print OUT "\tcall get_scav_parameters(temperature=temperature,cs_per_clust=cs)\n\n";
...
if($lfortran){
	...
	print OUT "subroutine get_coll$append_to_subroutine_names(";
	if($lloop){
		print OUT "nrates,";
	}
	print OUT "K";
...
if(!$disable_evap){
	...
	if($lfortran){
		print OUT "subroutine get_evap$append_to_subroutine_names(";
		if($lloop){
			print OUT "nrates,";
		}
		print OUT "E";
		...
		if($lhs_tests){
			print OUT "use acdc_hsdata, only : get_hsdata\n";
		}
		print OUT "\timplicit none\n";
		if(!$lloop){
...
################################################################################
		...
		if($loop_evap_coef =~ m/^DeltaG$/i){
			...
			if($variable_temp){
				...
				print OUT "\tcall get_coll$append_to_subroutine_names(nrates,K)\n";
			}
...
################################################################################
			...
			if($nmol_types_used>1){
				...
				if($lhs_tests){
					...
					print OUT "\t\t\tcall get_hsdata(ind,lhs(i),deltah(i),deltas(i))\n";
					print OUT "\t\tend do\n";
				}
				print OUT "\tend if\n\n";
			}
...
#	print DRIVER "\t\t\tform = nan;\n";
	...
	print DRIVER "\t\tif max(abs(Cf(1:$max_cluster_number)-C0(1:$max_cluster_number))./max(C0(1:$max_cluster_number),1e-6))<1e-6\n\t\t\tconverged = 1;\n";
	print DRIVER "\t\telseif length(T)>2 && max(abs(Cf(1:$max_cluster_number)-C(floor(length(T)*.75),1:$max_cluster_number))./max(C(floor(length(T)*.75),1:$max_cluster_number),1e-6))<1e-6\n\t\t\tconverged = 1;\n\t\tend\n";
    ...
    if($include_generic_neg && $include_generic_pos){
		if($generic_positive_ions_corr ne ''){
			print DRIVER "\t\tif abs(sum(Cf(positives))-sum(Cf(negatives)))/max(Cf($n_generic_pos),1e-6) > 1e-6\n";
			...
			print DRIVER "\t\tif abs(sum(Cf(positives))-sum(Cf(negatives)))/max(Cf($n_generic_neg),1e-6) > 1e-6\n";
			...
			print DRIVER "\t\tif abs(Cf($n_generic_pos)-sum(Cf(negatives)))/max(Cf($n_generic_pos),1e-6) > 1e-6\n";
			print DRIVER "\t\t\tCf($n_generic_pos) = sum(Cf(negatives));\n";
			print DRIVER "\t\t\tconverged = 0;\n";
			print DRIVER "\t\tend\n\n";
		}elsif($generic_negative_ions ne ''){
			print DRIVER "\t\tif abs(Cf($n_generic_neg)-sum(Cf(positives)))/max(Cf($n_generic_neg),1e-6) > 1e-6\n";
...
if($lhydrate_average==1){
	print "Done printing the output files. RH = $rh%\n";
}else{
